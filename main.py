...
def to_all_apps():
    ...
    for child in subframe_all_apps.winfo_children():
        child.destroy()
    if len(all_apps) == 0:
        Label(subframe_all_apps, text="Заявки отсутствуют!").pack(pady=10)
    for i in range(len(all_apps)):
        frames.append(Frame(subframe_all_apps, borderwidth=1, relief=SOLID))
        Label(frames[i], text=all_apps[i]["name"]).pack()
        Label(frames[i], text=f"{all_apps[i]['date_begin']}-{all_apps[i]['date_end']}").pack()
        Label(frames[i], text=f"Сумма: {all_apps[i]['asked_sum']}").pack()
        Label(frames[i], text="Статус: Ожидание").pack()
...
def to_create_app():
    ...
    for i in range(len(mems)):
        ...
        Label(rows_members[i], text=names_members[i]).pack(anchor=NE, side=LEFT, pady=5)
        Entry(rows_members[i], textvariable=roles_inputs[i]).pack(anchor=NE, side=LEFT, pady=5)
...
Label(subframe_auth_login, text="Логин: ").pack(anchor=NE, side=LEFT, pady=5)
ent_login_auth = Entry(subframe_auth_login)
...
Label(subframe_auth_password, text="Пароль: ").pack(anchor=NE, side=LEFT, pady=5)
ent_password_auth = Entry(subframe_auth_password, show="*")
ent_password_auth.pack(anchor=NW, side=LEFT, padx=5, pady=5)


lbl_error_auth = Label(frame_authorization)
lbl_error_auth.pack()

btn_login = Button(frame_authorization, text="Войти", command=process_signup)
btn_login.pack()
btn_to_register = Button(frame_authorization, text="Создать новую учетную запись", command=to_register)
...
Label(subframe_register_fio, text="ФИО: ").pack(anchor=NE, side=LEFT, pady=5)
ent_fio_registration = Entry(subframe_register_fio)
...
Label(subframe_register_login, text="Логин: ").pack(anchor=NE, side=LEFT, pady=5)
ent_login_registration = Entry(subframe_register_login)
...
Label(subframe_register_mail, text="Почта: ").pack(anchor=NE, side=LEFT, pady=5)
ent_mail_registration = Entry(subframe_register_mail)
...
Label(subframe_graduation, text="Ученая степень: ").pack(anchor=NE, side=LEFT, pady=5)
...
Label(subframe_register_password, text="Пароль: ").pack(anchor=NE, side=LEFT, pady=5)
ent_password_registration = Entry(subframe_register_password, show="*")
...
Label(subframe_register_password_repeat, text="Повторите пароль: ").pack(anchor=NE, side=LEFT, pady=5)
ent_password_repeat_registration = Entry(subframe_register_password_repeat, show="*")
ent_password_repeat_registration.pack(anchor=NW, side=LEFT, padx=5, pady=5)

lbl_error_register = Label(frame_registration)
lbl_error_register.pack()

btn = Button(frame_registration, text="Зарегистрироваться", command=process_register)
btn.pack()
...
btn_organisation = Button(subframe_nav_buttons, text="Организация", command=to_organisation)
btn_organisation.pack(anchor=NW, side=LEFT, padx=5, pady=5)
btn_all_apps = Button(subframe_nav_buttons, text="Заявки", command=to_all_apps)
btn_all_apps.pack(anchor=NW, side=LEFT, padx=5, pady=5)
btn_create_app = Button(subframe_nav_buttons, text="Создать заявку", command=to_create_app)
...
Label(subframe_org_name, text="Организация").pack(anchor=NE, side=LEFT, pady=5)
ent_name_org = Entry(subframe_org_name)
...
tbl_members.heading("graduation", text="Ученая степень")
tbl_members.heading("role", text="Роль")
tbl_members.pack(fill=BOTH, expand=1)

btn_delete_user = Button(subframe_organisation, text="Удалить выделенного пользователя", command=delete_selected_user)
...
Label(subframe_new_user, text="Имя нового пользователя: ").pack(anchor=NE, side=LEFT, pady=5, padx=10)
ent_new_user_name = Entry(subframe_new_user)
ent_new_user_name.pack(anchor=NW, side=LEFT, padx=5, pady=5)

Label(subframe_new_user, text="Роль: ").pack(anchor=NE, side=LEFT, pady=5)
ent_new_user_role = Entry(subframe_new_user)
...
Label(subframe_new_user, text="Ученая степень: ").pack(anchor=NE, side=LEFT, pady=5)
comb_graduation_new_user.pack(anchor=NW, side=LEFT, padx=5, pady=5)


btn_add_user = Button(subframe_organisation, text="Добавить пользователя", command=add_user)
btn_add_user.pack()
btn_save_users = Button(subframe_organisation, text="Сохранить", command=org_save_to_db)
btn_save_users.pack()
...
Label(subframe_create_app, text="Название проекта").pack(pady=5)
ent_app_name = Entry(subframe_create_app)
ent_app_name.pack()

Label(subframe_create_app, text="Тема").pack(pady=5)
ent_app_topic = Entry(subframe_create_app)
ent_app_topic.pack()

Label(subframe_create_app, text="Описание").pack(pady=5)
...
Label(subframe_create_app, text="Предметная область").pack()
comb_subject.pack()


Label(subframe_create_app, text="Сотрудники").pack(pady=5)
...
Label(subframe_create_app, text="Сроки выполнения исследований").pack(pady=5)
...
Label(subframe_date_begin, text="Начало").pack()
ent_date_begin.pack()
...
Label(subframe_date_end, text="Конец").pack()
ent_date_end.pack()

Label(subframe_create_app, text="Запрашиваемая сумма финансирования").pack(pady=5)
ent_app_sum = Entry(subframe_create_app)
ent_app_sum.pack()

lbl_error = Label(subframe_create_app)
lbl_error.pack()

btn_process_app_creation = Button(subframe_create_app, text="Создать", command=app_save_to_db)
